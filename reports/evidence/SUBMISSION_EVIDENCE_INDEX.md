...
| Deterministic scoring and optimizer fallback are implemented | fallback implementation + regression tests + architecture/runtime evidence | `PROVEN_BY_REPO` | Fallback is limited to documented non-definitive outcomes and preserves applicable deterministic constraints; proven `INFEASIBLE` remains definitive |
...
| Preproduction contract alignment is explicitly audited against the current production runtime | `docs/submission/PREPRODUCTION_CONTRACT_ALIGNMENT.md` + current runtime implementation + `docs/submission/IMPLEMENTATION_DECISION_INFEASIBLE_FALLBACK.md` | `PROVEN_BY_REPO` | GAP-01, GAP-02, and DEV-01 resolved; historical toolchain wording remains preserved and DEV-01 is recorded as an explicit more-conservative semantic refinement |
| A proven `INFEASIBLE` CP-SAT result is not converted into successful deterministic fallback allocation | `docs/submission/IMPLEMENTATION_DECISION_INFEASIBLE_FALLBACK.md` + production optimizer behavior + INFEASIBLE regression test + architecture overview | `PROVEN_BY_REPO` | `INFEASIBLE` remains definitive, selected rescue allocation remains empty, planning quantity remains unallocated, and human exception review is required |
| Rescue Decision Report explicitly separates supported recovery, rescue, waste, loss, social-allocation, logistics-budget, capacity-utilization, and BALANCED rescue-floor fields | report contract + production report builder + optimizer outputs + report regression tests | `PROVEN_BY_REPO` | Decision-support and configured-runtime evidence only; not realized real-world recovery, impact, loss reduction, or capacity utilization |
