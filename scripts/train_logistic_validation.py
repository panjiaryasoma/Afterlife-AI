"""Train LR on train only and score validation."""
...
def main() -> None:
    """Fit LR and evaluate validation without accessing test."""

    config = load_modeling_config(MODELING_CONFIG_PATH)
    ...
    train = select_modeling_split(
        modeling,
        config.development_policy.fit_split,
    )
    validation = select_modeling_split(
        modeling,
        config.development_policy.selection_split,
    )
    ...
    predictions = predict_logistic_probabilities(
        model,
        validation,
        contract=contract,
    )
    ...
    print("Logistic Regression validation complete")
