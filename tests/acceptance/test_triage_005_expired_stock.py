...
def test_triage_005_expired_stock_is_excluded_from_rescue() -> None:
    lot = RawInventoryLot(
        ...
        normal_selling_price=Decimal("3500"),
        source_location="STORE-01",
        expiry_date=date(2026, 8, 3),
        ...
        units_sold_observation_window=Decimal("60"),
        observation_days=30,
    ...
    assert result.expired_quantity == Decimal("12")
    assert result.review_quantity == Decimal("0")

    assert result.inventory_status is InventoryStatus.EXPIRED
    assert result.surplus_source is None
    assert result.triage_reason_codes == [
        "EXPIRED_HARD_REJECT"
    ]

    # Expiry harus menang sebelum kalkulasi sales-based routing.
