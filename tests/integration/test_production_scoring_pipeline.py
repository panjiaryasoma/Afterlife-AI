...
def test_production_pipeline_scores_gate_eligible_candidates() -> None:
    ...
    assert all(
        candidate.estimated_rescue_success_score is not None
        for candidate in scored
    )

    assert all(
        0 <= candidate.estimated_rescue_success_score <= 1
        for candidate in scored
        if candidate.estimated_rescue_success_score is not None
    )
