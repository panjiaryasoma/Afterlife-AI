...
def _shared_capacity_partner_registry(
    tmp_path: Path,
) -> Path:
    target = (
        tmp_path
        / "partner_registry_shared_capacity.yaml"
    )
...
def test_production_optimizer_enforces_shared_partner_capacity_across_lots(
    ...
    partner_registry_path = (
        _shared_capacity_partner_registry(
            tmp_path
        )
    )
    ...
    result = run_production_pipeline(
        ...
        request_id=(
            "PRODUCTION-SHARED-PARTNER-CAPACITY"
        ),
    )
    ...
    partner_allocations = [
        allocation
        for allocation
        in result.report.selected_allocations
        if (
